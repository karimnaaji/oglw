...
vec4 fxaa(sampler2D sampler, vec2 fragCoord, vec2 resolution,
...
vec4 applyFXAA(sampler2D sampler, vec2 fragCoord, vec2 res) {
    ...
    //compute FXAA
    return fxaa(sampler, fragCoord, res, v_rgbNW, v_rgbNE, v_rgbSW, v_rgbSE, v_rgbM);
}

void main(void) {
    outColour = applyFXAA(tex, gl_FragCoord.xy, resolution);
}
