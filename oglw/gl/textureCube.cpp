...
void TextureCube::load(const std::string& _file) {
    ...
    for (int y = 0; y < height; ++y) {
        ...
        for (int iFace = 0; iFace < 4; ++iFace) {
            ...
            if (iFace == 2 && jFace == 1)
                face = &m_faces[0]; // POS_X
            if (iFace == 0 && jFace == 1)
                face = &m_faces[1]; // NEG_X
            if (iFace == 1 && jFace == 0)
                face = &m_faces[2]; // POS_Y
            if (iFace == 1 && jFace == 2)
                face = &m_faces[3]; // NEG_Y
            if (iFace == 1 && jFace == 1)
                face = &m_faces[4]; // POS_Z
            if (iFace == 3 && jFace == 1)
                face = &m_faces[5]; // NEG_Z
...
void TextureCube::update(GLuint _textureUnit) {
    ...
    for (int i = 0; i < 6; ++i) {
        Face& f = m_faces[i];
        glTexImage2D(CubeMapFace[i], 0, m_options.m_internalFormat, m_width, m_height, 0, m_options.m_format,
                     GL_UNSIGNED_BYTE, f.m_data.data());
        f.m_data.clear();
    }
}
